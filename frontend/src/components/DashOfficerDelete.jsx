...
export const DashOfficerDelete = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        console.log(error);
        setStatus({ type: "failure", message: "Failed to load officers." });
      }
    };
    if (userIdToDelete === "") {
      fetchUsers();
  ...
  const getDeleteUser = async () => {
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        return setStatus({ type: "failure", message: data.message || "Officer not found." });
      }
      setUsers([data]);
      setStatus(null);
    } catch (error) {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setStatus({ type: "failure", message: data.message || "Failed to delete officer." });
        return;
      }

      setStatus({ type: "success", message: typeof data === "string" ? data : "Officer deleted successfully." });
      setUserIdToDelete("");
